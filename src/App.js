...
import twitReducer from "./util/twitReducer";

function App() {
  // const [twits, setTwits] = useState([]); // 트윗 목록 저장
  const [twits, dispatch] = useReducer(twitReducer, []);
  ...
  useEffect(() => {
    axios.get(`https://randomuser.me/api/?results=${Math.floor(Math.random() * 100)}`).then((res) => {
      dispatch({
        type: "INIT", data: res.data.results.map((user, i) => ({
          id: i,
